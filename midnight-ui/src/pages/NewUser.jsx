...
export default function NewUser() {
  ...
  const handleAutoID = async () => {
    ...
    try {
      ...
      // Fetch user immediately after write
      const snapshot = await getDoc(docRef);
      if (snapshot.exists()) {
        const userData = snapshot.data();
        setCurrentUser({ id: docRef.id, ...userData });
        localStorage.setItem("currentUserId", docRef.id);
        setMessage(`User created: ${userData.name}`);
  ...
  const handleCustomID = async () => {
    ...
    try {
      ...
      await setDoc(userRef, { name, faction, kudos: initialKudos });
